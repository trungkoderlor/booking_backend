document.addEventListener("DOMContentLoaded", () => {
  ...
  if (uploadImage) {
    ...
    uploadImageInput.addEventListener("change", (e) => {
      const cancelImageButton = uploadImage.querySelector(
        "[cancel-image-button]"
      );
  ...
  if (sort) {
    ...
    if (sortKey && sortValue) {
      ...
      const optionSelected = selectorSort.querySelector(
        `option[value='${stringSelected}']`
      );
      if (optionSelected) {
  ...
  //end sort
  //create doctor checkbox
  const roleSelect = document.getElementById("role_id");
  if (roleSelect) {
    const doctorGroup = document.querySelectorAll(".doctor-group");
    roleSelect.addEventListener("change", function () {
      if (this.value == "R2") {
        doctorGroup.forEach((checkbox) => {
